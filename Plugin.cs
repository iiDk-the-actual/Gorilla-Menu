...
namespace GorillaMenu
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        void PlayerSpawned()
        {
            ...
            lr = GTPlayer.Instance.gameObject.AddComponent<LineRenderer>();
            lr.material = new Material(Shader.Find("Sprites/Default"));
            lr.startColor = Color.white;
            lr.endColor = Color.white;
            lr.startWidth = 0.02f;
            lr.endWidth = 0.02f;
            rHand = GameObject.Find("Player Objects/Local VRRig/Local Gorilla Player/GorillaPlayerNetworkedRigAnchor/rig/body/shoulder.R/upper_arm.R/forearm.R/hand.R").GetComponent<Transform>();
            testCube = new GameObject("RayCube");
        ...
        void Update()
        {
            ...
            if (menuasset == null && Camera.main != null && GameObject.Find("Player Objects/Local VRRig/Local Gorilla Player") != null)
            {
                PlayerSpawned();
            }
            ...
            {
                menu.SetActive(true);
                menuParent.transform.position = GTPlayer.Instance.headCollider.transform.position;
                menuParent.transform.eulerAngles = new Vector3(0, GTPlayer.Instance.headCollider.transform.eulerAngles.y, 0);
                ypos = GTPlayer.Instance.headCollider.transform.position.y;
                isMenuOpen = true;
            }
            if (leftSec)
            {
                menuParent.transform.position = Vector3.Lerp(menuParent.transform.position, GTPlayer.Instance.bodyCollider.transform.position, 8 * Time.deltaTime);
        ...
        public void AddMod(string name, GameObject ui)
        {
            ...
            modButton.name = name;
            modButton.transform.GetChild(1).GetComponent<TextMeshProUGUI>().text = name.ToUpper();
